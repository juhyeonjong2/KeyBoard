...
<%
  ...
  Member member = (Member)session.getAttribute("login");
	...
	if(db.connect())
	{	//이거 업데이트 문으로 수정하고 로그인ok를 delyn이 y라면 로그인이 안되게 수정 시키자
		// 그렇다면 업데이트할때 필요한 정보는 아이디 비번 넘버인건 똑같다.
		 String sql = "DELETE FROM member WHERE mno = ? and mid = ? and mpw = md5(?) ";
		 int count = db.setString(sql).setInt(mno2).setString(mid).setString(mpw).update();
